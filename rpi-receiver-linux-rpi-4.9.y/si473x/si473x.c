				    ...
				    unsigned int fmt)
{
	...
	err = snd_soc_update_bits(codec_dai->codec,
				  ...
				  struct snd_soc_dai *dai)
{
	...
	err = snd_soc_update_bits(dai->codec, SI473X_PROP_DIGITAL_OUTPUT_FORMAT,
	...
	err = snd_soc_write(dai->codec, SI473X_PROP_DIGITAL_OUTPUT_SAMPLE_RATE,
			    rate);
	if (err < 0) {
				  ...
				  struct snd_soc_dai *dai)
{
	...
	err = snd_soc_write(dai->codec, SI473X_PROP_DIGITAL_OUTPUT_SAMPLE_RATE,
			    0);
	if (err < 0)
		dev_err(dai->codec->dev, "Failed to disable digital audio output\n");
...
static int si473x_set_sysclk(struct snd_soc_codec *codec, int clk_id,
	int source, unsigned int freq, int dir)
{
	struct si473x_core *core = i2c_mfd_cell_to_core(codec->dev);
	int err;
	...
	refclk_freq = snd_soc_read(codec, SI473X_PROP_REFCLK_FREQ);
	if (refclk_freq == -1)
		dev_err(codec->dev, "Failed to read REFCLK_FREQ\n");

	prescaler = freq / SI473X_PROP_REFCLK_FREQ_MAX + 1;
	while (freq % prescaler) {
		prescaler++;
		if (prescaler > freq / SI473X_PROP_REFCLK_FREQ_MIN) {
			dev_err(codec->dev, "Failed to calculate REFCLK_PRESCALE and REFCLK_FREQ\n");
			return -EINVAL;
		}
	}
	...
	err = snd_soc_write(codec, SI473X_PROP_REFCLK_FREQ,
			    refclk_freq);
	if (err < 0)
		dev_err(codec->dev, "Failed to set REFCLK_FREQ\n");
	err = snd_soc_write(codec, SI473X_PROP_REFCLK_PRESCALE,
			    freq / refclk_freq);
	if (err < 0)
		dev_err(codec->dev, "Failed to set REFCLK_PRESCALE\n");
...
static struct regmap *si473x_get_regmap(struct device *dev)
{
	return dev_get_regmap(dev->parent, NULL);
}

static struct snd_soc_codec_driver soc_codec_dev_si473x = {
	.set_sysclk = si473x_set_sysclk,
	.get_regmap = si473x_get_regmap,
	.component_driver = {
		.controls = si473x_snd_controls,
		.num_controls = ARRAY_SIZE(si473x_snd_controls),
...
{
	return snd_soc_register_codec(&pdev->dev, &soc_codec_dev_si473x,
				      &si473x_dai, 1);
}

static int si473x_platform_remove(struct platform_device *pdev)
{
